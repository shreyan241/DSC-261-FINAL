...
class Adversarial_Kernel_SHAP_Model(Adversarial_Model):
	...
	def train(self, X, y, feature_names, background_distribution=None, perturbation_multiplier=10, n_samples=2e4, rf_estimators=100, n_kmeans=10, estimator=None,
				dummy_idcs = [], integer_idcs = [], custom_n=100):
		...
		# In case of treeEnsemble and rbfDataGen we just load the whole distribution set
			...
			if self.generator == "RBF":
				if self.generator_specs["experiment"] == "Compas":
					...
					X_gen = pd.read_csv("../Data/shap_german_adversarial_train_RBF.csv")
				else:
					X_gen = pd.read_csv("../Data/shap_cc_adversarial_train_RBF.csv")
			elif self.generator == "Forest":
				if self.generator_specs["experiment"] == "Compas":
					X_gen = pd.read_csv("../Data/shap_compas_adversarial_train_forest.csv")
				elif self.generator_specs["experiment"] == "German":
					X_gen = pd.read_csv("../Data/shap_german_adversarial_train_forest.csv")
				else:
					X_gen = pd.read_csv("../Data/shap_cc_adversarial_train_forest.csv")
			else:
				if self.generator_specs["experiment"] == "Compas":
					X_gen = pd.read_csv("../Data/shap_compas_adversarial_train_CTGAN.csv")
				elif self.generator_specs["experiment"] == "German":
					X_gen = pd.read_csv("../Data/shap_german_adversarial_train_CTGAN.csv")
				else:
					X_gen = pd.read_csv("../Data/shap_cc_adversarial_train_CTGAN.csv")
